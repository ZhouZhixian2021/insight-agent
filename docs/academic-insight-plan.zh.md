...
- M1 已提供固定 Research Brief、首请求自动计划模式和执行前人工批准。该里程碑剩余的验收缺口是真实模型 Web 场景录制；此后主要产品缺口是持久证据流水线。
...
| M1 — 稳定研究输入（实现完成） | 意图等价时，简短请求和详细请求解析为同一份明确研究规格。 | 固定 Research Brief、首请求自动计划模式、一次实质性澄清、可见审核和批准后执行边界。 | 聚焦生命周期与预设测试通过；仍需录制一个真实模型 Web 场景。 | M0。 |
