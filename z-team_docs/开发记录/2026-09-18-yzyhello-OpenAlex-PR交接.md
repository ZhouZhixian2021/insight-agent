...
- 修复 arXiv 截断标记：解析 Atom `<opensearch:totalResults>`，超出实际返回条目数时如实置 `truncated`；`parseArxivFeed` 改为返回条目与计数，缺失或畸形计数为 null。
...
- arXiv 截断修复定向回归：source-arxiv 2 个测试文件 17 项通过；TypeScript 编译、定向 oxlint、导出 JSDoc、arxiv 双语配对通过。`test:docs` 14/15，唯一失败为 CVF 双语对，来自本地未提交改动，不在本 PR 范围。
...
- arXiv `truncated` 依赖 feed 的 `totalResults`；元素缺失或畸形时保持 `false`，即无法证明截断时不虚报，个别异常响应仍可能漏报。
...
- [arXiv 截断标记修复](2026-09-18-B-arXiv截断标记修复.md)
