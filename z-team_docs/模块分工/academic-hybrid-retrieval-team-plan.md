...
本文记录“通用 Web 搜索 + 多个学术源”的第一版团队实施计划。A-H1 与 A-H2 已合并，B-H1 的 Web 论文引用识别也已合并；A-H3a 已在个人分支完成工作流执行器与聚焦验证，仍待合并。产品尚未执行混合检索，真实接入仍依赖 B-H2 的权威核验和 A-H3b/A-H4。A、B、C 沿用[学术洞察模块分工](academic-module-ownership.md)中的目录所有权；临时代办也按本文的模块边界拆分提交，避免把来源、工作流和界面逻辑写入同一目录。
...
**当前进度：已合并。** 第 3 版计划要求每条查询携带检索策略；第 1、2 版继续兼容。A-H3 接入前，第 3 版计划可以预览与审核，但运行会明确拒绝，避免忽略已批准策略。
...
**当前进度：A-H3a 已在个人分支完成，待合并。** 工作流执行器已固定并发双通道、精确引用去重、Provider 白名单、独立核验预算、单边失败保留与取消语义，并用替身核验接口完成聚焦测试。A-H3b 等待 B-H2 提供真实的按 Provider 搜索与引用核验操作后接入 Controller；在此之前保留第 3 版计划运行保护。
