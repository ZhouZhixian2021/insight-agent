...
本文记录“通用 Web 搜索 + 多个学术源”的第一版团队实施计划。A-H1 共享接口已于 2026-09-22 在 A 的个人分支完成实现与聚焦验证，尚待提交、PR 和合并；产品仍未执行混合检索。A、B、C 沿用[学术洞察模块分工](academic-module-ownership.md)中的目录所有权；临时代办也按本文的模块边界拆分提交，避免把来源、工作流和界面逻辑写入同一目录。
...
来源专用引用必须检查官方主机名和受支持的论文路径结构，不根据相似域名、任意链接或标题猜测。非法或含糊值返回明确失败，不猜测缺失字符；任意 BibTeX Key 不作为全局唯一标识。官方目录页、搜索页与论文页必须区分。
