...
New structured plan handoffs use `schemaVersion: 3` and require `searchPlan`: each entry has `query`, a Chinese `purpose`, `questions` matching the Brief, and a `retrieval` policy reviewed with the query. Every research question needs coverage. The policy selects `academic` and/or `web_discovery`, allows only OpenAlex/arXiv for direct search and OpenAlex/arXiv/ACL/PMLR/CVF for reference verification, and carries Web-discovery and verification bounds capped at eight. Channel-dependent provider lists and bounds must agree, and duplicate values, unsupported values, unknown fields, negative values, and over-limit values are rejected before review. The Controller projects this handoff into the existing version-1 domain Brief and separate pipeline searches; the model package is unchanged. Legacy version-1 and version-2 plans remain readable, but a plan without searches cannot preview or run: the user is asked to have the system complete and reapprove it. No additional model request generates queries at run time.

The public Remote vocabulary keeps the per-search `retrieval` policy optional so version-1/version-2 plans remain readable, and reserves an optional run-level `hybridRetrieval` projection. The policy distinguishes `academic` from `web_discovery`, direct-search providers from reference-verification providers, and separate Web-discovery and verification bounds. The run projection carries producer-settled hybrid stages, separate URL/reference/work counts, and sanitized reference outcomes. Absence means “hybrid policy not supplied,” not a zero-valued hybrid run. Until A-H3 connects policy-aware execution, preview exposes an approved version-3 policy but `run` rejects it rather than silently executing the old source selection.
...
- Hybrid Web discovery is not executed yet. Version-3 plans are reviewable but cannot run until A-H3 connects the approved policy to the workflow; version-1/version-2 Academic-only plans retain their existing execution path.
