...
The public Remote vocabulary also reserves an optional per-search `retrieval` policy and an optional run-level `hybridRetrieval` projection. The policy distinguishes `academic` from `web_discovery`, direct-search providers from reference-verification providers, and separate Web-discovery and verification bounds. The run projection carries producer-settled hybrid stages, separate URL/reference/work counts, and sanitized reference outcomes. Current schema-version-2 plans and single-channel runs omit both fields; their absence means “hybrid policy not supplied,” not a zero-valued hybrid run. A later plan/workflow increment will populate them without changing the fixed browser contract.
...
- Hybrid Web discovery is not executed yet. The optional plan and result fields are an interface baseline for the next plan, workflow, provider, and client increments.
