...
| `maxAttempts` | `1` | Total attempts for network failures or per-attempt timeouts; valid range `1`–`3` |
| `retryDelayMs` | `0` | Delay before another network attempt; maximum `30000` |
...
The provider distinguishes cancellation, timeout, rate limit, malformed JSON/metadata, network failure, and other HTTP errors. It repeats only network failures and its own per-attempt timeout within the configured attempt bound; HTTP responses, parsing failures, rate limits, and caller cancellation are not retried. `searchAll()` records source failures without discarding other sources. A date filter can exclude an old paper when OpenAlex's merged record uses a later publication date; leave it unset when discovery must not exclude such records, then verify eligibility against authoritative metadata. All `firstPublicDate` values remain unknown rather than treating aggregate dates as verified first-public dates.
...
[Normalization](src/normalize.ts) validates the upstream response and avoids substituting preprint URLs for published-version content. [The provider](src/provider.ts) retains URL candidates in a bounded instance-local map. No invariant companion is published because the provider owns no independently observable relationship that can diverge. Unit tests cover mapping and cancellation; the YAML loader test covers composition and a result snapshot. The opt-in `ACADEMIC_OPENALEX_LIVE=1` e2e records the fixed long queries and checks a separately identified BERT title/full-text control; it does not certify fixed-query recall or a research report.
...
- **Incomplete discovery** — index coverage and ranking may omit required papers; the provider performs no planning or pagination, and retries only transient transport failures when explicitly configured. Semantic search availability is upstream-dependent.
