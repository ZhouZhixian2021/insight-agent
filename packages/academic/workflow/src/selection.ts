...
import type { SelectedPaper } from './pipeline-types.ts'
 ...
 * @returns selected papers in search order, capped by the approved included-work limit.
 */
export function selectResearchPapers(
  ...
  for (const work of ingested.works) {
    if (selected.length >= brief.stopConditions.maximumIncludedWorks) break
