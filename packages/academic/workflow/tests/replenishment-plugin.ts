  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    execute: async () => {
      ...
      const brief = { ...sample.brief, topic: 'Synthetic replenishment', questions: ['Compare the observed methods.'],
        ...
        evidenceRequirements: { ...sample.brief.evidenceRequirements, minimumIncludedWorks: 2, minimumFulltextWorks: 2 },
        stopConditions: { ...sample.brief.stopConditions, maximumCandidateWorks: 5, maximumIncludedWorks: 2,
      ...
      const result = await runResearchDraft({ brief, searches: [{ query: 'synthetic' }], synthetic: true }, {
      ...
      assert.equal(attempted.length, 4, JSON.stringify({ papers: result.papers.map(paper => paper.status),
        failures: result.failures, synthesis: result.synthesis }))
      assert.equal(result.retrievalRun.coverageSummary.includedWorks, 2)
      assert.equal(result.failures.length, 1)
      assert.equal(result.synthesis.status, 'completed')
      assert.ok(result.report)
      return JSON.stringify({ attempted, papers: result.papers.map(paper => paper.status),
