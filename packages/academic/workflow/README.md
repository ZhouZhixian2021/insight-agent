...
Callers supply search, selectPapers, fetcher, generator and now. search can adapt ctx.academicSource.search and fetcher can adapt ctx.web.fetch. `selectResearchPapers()` applies deterministic version, date, work-type, retraction, preprint and count rules through a provider-owned full-text resolver; natural-language inclusion and exclusion rules are reviewed against fetched content by the generator. Generator and clock are explicit; this library reads no credentials, creates no network client and introduces no generic scheduler.

Search results respect maximumCandidateWorks and the request bound; selection respects maximumIncludedWorks. Unapproved input, duplicate works, unknown versions and excluded/retracted selections reject. Hash conflicts return pauses; model scope exclusions retain their reason without contributing evidence. Acquisition or extraction failures retain the version and failed stage while other papers continue. Reports disclose exclusions, pauses, failures and truncation without claiming complete coverage.
