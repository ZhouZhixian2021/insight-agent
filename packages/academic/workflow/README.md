...
Callers supply search, selectPapers, fetcher, generator and now. search can adapt ctx.academicSource.search and fetcher can adapt ctx.web.fetch; selectPapers owns approved date, scope and inclusion/exclusion rules and returns reconciled version IDs with candidate URLs. Generator and clock are explicit; this library reads no credentials, creates no network client and introduces no generic scheduler.

Search results respect maximumCandidateWorks and the request bound; selection respects maximumIncludedWorks. Unapproved input, duplicate works, unknown versions and excluded/retracted selections reject. Hash conflicts return pauses; acquisition or extraction failures retain the version and failed stage while other papers continue. Reports disclose pauses, failures and truncation without claiming complete coverage.

Output status=completed means only that this pass finished, not that research or review is sufficient. report always uses draft mode and empty semantic reviews; report.evaluation carries quality status. Cancellation returns cancelled with completed papers and failures and no report; search failure rejects. Callers own durable model/network records and deadline cancellation signals.

Automatic repeated search, retries, cross-round index recovery, saturation rules and stopping as soon as evidence counts are met remain subsequent work. This pass has no automatic abstract fallback, main Web integration or final publication.

## Model response validation

`parseEvidenceDrafts(text)` accepts a complete JSON array of at most six entries and returns B's EvidenceDraft values. It validates the result bound, all six card sections, required fields, enums and Availability values before returning any drafts. Empty arrays and empty cardItems are accepted; excess entries, unknown fields, invented identities, failed availability and malformed responses throw EvidenceError with code EVIDENCE_INVALID_MODEL_OUTPUT. Diagnostics identify a field path without copying response content.
...
The returned PaperEvidenceGenerator accepts B's request plus parsed provenance and can be assigned directly to runResearchDraft's generator. Existing single-argument generators remain valid. B receives its unchanged EvidenceGenerationRequest; the A-owned wrapper associates the call with the work, version, content hash, source and extraction method.

Application callers use `runAcademicResearchDraft({ ctx, session, model, input, adapters, signal })`. This stable entry checks the exact model route before search or acquisition, defaults an omitted reasoning effort to `low`, and preserves an explicit caller choice. Unsupported reasoning fails before external paper work begins. The result includes `sessionId` alongside the per-paper states, failures, analysis and report so callers can locate the durable model records.
...
- This library provides a single-pass draft pipeline, paper handoff and opt-in model adapter. Session logging covers model requests and settlements, not complete workflow recovery or persisted evidence/card identities. Main Web entry wiring, semantic review, long-paper chunking and automatic retries remain subsequent work. It does not prove scholarly identity or distinguish format changes from content revisions.
