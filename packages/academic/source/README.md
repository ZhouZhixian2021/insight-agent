...
All search methods return normalized works and enforce the total `request.maxResults` bound. `searchAll()` uses configured discovery providers or every usable provider when unset. `searchProviders(request, providerIds, signal)` searches exactly the requested ids regardless of discovery configuration; empty, duplicate, missing, and unavailable selections fail before network access. Multi-provider searches keep successful works in `batch.items` when another provider fails, record that failure in `batch.failures`, and report actual calls, pre-bound record counts, truncation, and source limitations. `resolveFullText()` maps a selected version's source record back to its provider-owned ordered URL candidates. Calls accept an optional `AbortSignal` forwarded to providers.
