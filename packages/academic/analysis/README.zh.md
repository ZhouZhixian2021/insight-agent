...
调用方通过 `prepareAnalysisInput()` 按学术成果和实际内容版本整理证据卡片条目。结果保留有效条目、支持记录、来源定位和带位置的问题。这个纯函数库不执行检索、模型请求、持久化或 Claim 生成。
...
- 分析模块内部的整理类型不是共享 Claim 或报告类型。结论生成和语义评估等待共享模型的分析记录。
