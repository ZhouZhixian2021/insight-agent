...
The provider reuses the shared catalog fetch, filtering, and normalization helper. Its parser extracts each paper block and abstract path; the provider derives the matching canonical PDF path. The provider declares its catalog-only coverage through the `limitations` field, surfaced by `searchAll()`. Deduplication and body parsing remain in `academic-ingestion` and `academic-evidence`.
