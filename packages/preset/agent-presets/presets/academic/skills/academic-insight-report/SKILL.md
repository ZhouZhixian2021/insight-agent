...
Generate the structured `searchPlan` while preparing the Brief. Link each query to its exact research questions, give its purpose in Chinese, and attach the template's `retrieval` policy. Describe the same directions, enabled channels, direct Academic providers, verification providers, and Web/verification budgets in the readable Chinese plan. Cover every question within the template's query and round limits. Choose supported fields for the user; do not ask the user to enter queries, provider ids, enum values, or JSON. The Host executes these approved expressions and policies after approval. Source filtering still enforces dates, version states, and inclusion rules after candidate discovery, and Web snippets never become academic evidence.
