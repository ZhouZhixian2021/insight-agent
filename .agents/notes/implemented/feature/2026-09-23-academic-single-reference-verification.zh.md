...
Provider 没有全文候选或候选解析失败时，官方成果仍保留已核验状态。`fullTextFailure` 在 `fullText: null` 旁记录 `fulltext_unavailable` 或分类后的解析错误；官方记录不存在则以 `not_found` 判定核验失败。搜索批次和引用失败只将限流、超时和网络错误标记为可重试。非预期的 Provider 错误使用通用消息，不暴露原始文本。

## 考虑过的替代方案

只接受看似有效的 URL 或成功的 HTTP 响应，无法核验论文元数据。为每条 Web 结果重新搜索完整目录会增加工作量，也可能漏掉未列入配置目录的论文。读取单篇页面后仍使用推导出的 PMLR PDF 路径，可能指向不存在的文件。把全文候选缺失当作论文不存在，会丢掉已核验元数据，并隐藏无法生成证据的原因。

## 后果

五个 Provider 可结算一条已识别引用，而不扩展其引用网络。官方记录缺失、传输错误、限流、解析失败和全文候选缺失保持区分；调用方取消会中止操作。只有元数据的成果仍可进入摄取，但不能提供全文证据。全文安全、下载限制、解析和证据生成仍由下游抓取与证据包负责。
