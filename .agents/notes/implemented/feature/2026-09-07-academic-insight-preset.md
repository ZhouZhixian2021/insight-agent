...
The Academic plan template and planning instructions require a Simplified Chinese title, headings, explanations, and readable body. The Agent turns everyday questions into a bounded scope, research questions, and executable requirements, discloses assumptions, and generates search aliases and version-state values itself. Every consequential execution setting remains visible in the readable plan. The Web review folds the single named structured handoff while preserving the original plan and approval identity; it does not translate or rewrite already approved plans.
