...
6. **Verified Web provenance** stays on the contributing `IngestRecord`, outside bibliographic fields. The hybrid workflow supplies the discovery URL and the Academic Provider that verified its reference; when duplicate provider records collapse to one version, distinct URL/provider pairs remain attached to that version. The outcome projects them with the retained work and version ids. Ingestion does not treat an unverified Web result as a scholarly record.

The library consumes `IngestRecord` (a `{ academicWork, workVersion }` pair with optional verified discoveries), which remains structurally compatible with the source seam's `AcademicSourceWork`, so provider output flows in without a dependency on `dsh-academic-source`.
