...
runAcademicResearchDraft 是面向应用的对象参数入口。它在外部论文处理前预检准确模型路由，推理强度未指定时默认使用 low，保留调用方明确选择，并随流水线结果返回持久化 Session ID。因此，不支持的推理强度会在检索前明确失败，而不会变成单篇抽取失败。该入口仍不替调用方选择提供方、模型、选文策略或发布目标。
