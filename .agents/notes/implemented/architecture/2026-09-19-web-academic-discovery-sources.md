...
ACL Anthology, CVF, and PMLR remain mounted because their registered records can resolve known full-text locations. They are not members of the Web discovery list. OpenAlex receives no publication-year filter: its aggregate publication date does not establish `first_public_release`, and strict date eligibility continues to require an authoritative source. The source runtime still performs one unchanged query per provider without planning or retries.
