...
固定合成夹具覆盖 DOI、arXiv、ACL、PMLR 与 CVF 引用，包括没有 DOI 或 arXiv ID 的官方记录。标识原始写法必须能从候选输入中还原；规范化不能创造候选中不存在的大小写。夹具还覆盖三种识别问题、一条核验成功、一条分类失败和浏览器投影。计划会为第 3 版交接填充策略。工作流的 `executeHybridSearch()` 同时启动获批的学术源发现和 Web 发现，执行精确引用去重、Provider 白名单与核验预算，只返回直接发现或核验成功的论文及彼此分离的渠道观察值。重复引用只核验一次，但保留每个发现 URL；摄取库在应用候选成果上限前归并精确关联的记录，并保留每个纳入成果的全部版本。Controller 使用 `approvedSearchAdapter()` 将已批准查询接到 Session 的 `searchProviders()`、`web.search()` 与 `verifyReference()`；旧计划使用 `searchAll()`。流水线向这些操作传递统一的候选上限、时间预算和取消信号。Web 搜索与核验失败进入既有发现阶段和失败列表。A-H4 把已完成查询与 ingestion 事实投影为固定 Remote 字段。URL／引用／尝试数累加已完成查询，合并重复数为整轮上限应用前实际进入 ingestion 的记录数减去不同论文数。计数保留单查询版本归并前的原始贡献记录；重复引用与被排除的记录不算论文合并。中断查询增加限制说明，不编造来源失败；没有完成查询时不提供混合投影。发现 URL 移除凭据、查询参数和片段；网页片段与原始诊断留在内部。无密钥 SDK 会话回放固定真实 Loader 下的适配调用与结果；Controller 聚焦测试覆盖核验论文进入真实选文、全文和报告流水线。
