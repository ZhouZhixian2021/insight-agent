...
Controller 负责这份编排交接，单独提取既有第 1 版领域 Brief，因此 Academic model 类型保持不变。第 1、2 版历史交接仍可读取；缺少检索方案时，必须补齐并重新批准才能开始研究。系统不从主题猜测缺失查询，也不悄悄修改已批准要求。A-H3b 适配器通过混合检索执行器落实第 3 版策略，保留批准的渠道、Provider 和上限；没有策略的历史检索继续使用部署配置的学术检索。
