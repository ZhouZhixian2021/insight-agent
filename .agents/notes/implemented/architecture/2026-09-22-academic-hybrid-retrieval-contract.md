...
The fixed synthetic fixture covers DOI, arXiv, ACL, PMLR, and CVF references, including official records without DOI or arXiv identifiers. Raw identifier spelling remains recoverable from its candidate; normalization cannot invent case that the candidate does not contain. The fixture also covers all three identification issue codes, one verified result, one classified failure, and the browser projection. Planning populates the policy for schema-version-3 handoffs. The workflow's `executeHybridSearch()` starts approved Academic and Web discovery together, applies exact-reference deduplication, Provider allowlists and verification budgets, and returns only direct or verified works beside separate channel observations. Repeated references require one verification but keep each discovery URL; ingestion merges exact-linked records before the candidate-work limit and retains every version of each included work. The Controller uses `approvedSearchAdapter()` to bind approved queries to the Session's `searchProviders()`, `web.search()`, and `verifyReference()`; legacy plans use `searchAll()`. The pipeline forwards its common candidate bound, time budget, and cancellation signal to these operations. Web-search and verification failures enter the existing discovery stage and failure list. A-H4 projects completed queries and ingestion facts into the fixed Remote fields. URL/reference/attempt counts sum completed queries, while merged duplicates count admitted records minus distinct ingested works before the run-wide cap. Original contributions survive per-query version reconciliation for counting; repeated references and discarded records are not work merges. Interrupted queries contribute a limitation rather than fabricated source failures; no completed query means no hybrid projection. Discovery URLs drop credentials, query strings and fragments; snippets and raw diagnostics remain internal. A keyless SDK session replay pins the adapter calls and results under the real Loader; focused Controller tests cover verified papers entering the real selection, full-text, and report pipeline.
