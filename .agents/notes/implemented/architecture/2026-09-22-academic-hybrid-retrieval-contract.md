...
The fixed synthetic fixture covers DOI, arXiv, ACL, PMLR, and CVF references, including official records without DOI or arXiv identifiers. Raw identifier spelling remains recoverable from its candidate; normalization cannot invent case that the candidate does not contain. The fixture also covers all three identification issue codes, one verified result, one classified failure, and the browser projection. Planning populates the policy for schema-version-3 handoffs. The workflow's `executeHybridSearch()` starts approved Academic and Web discovery together, applies exact-reference deduplication, Provider allowlists and verification budgets, and returns only direct or verified works beside separate channel observations. The Controller still rejects schema-version-3 execution until Academic Source supplies the call-scoped provider operations and A-H4 projects the observations.
