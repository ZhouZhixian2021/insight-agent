...
The fixed synthetic fixture covers DOI, arXiv, ACL, PMLR, and CVF references, including official records without DOI or arXiv identifiers. Raw identifier spelling remains recoverable from its candidate; normalization cannot invent case that the candidate does not contain. The fixture also covers all three identification issue codes, one verified result, one classified failure, and the browser projection. Planning now populates the policy for schema-version-3 handoffs, while run execution still rejects that policy and does not populate the run projection until A-H3/A-H4 consume this contract.
